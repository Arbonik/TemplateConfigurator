    ...
    Row(modifier = Modifier.fillMaxSize()) {
        ...
        Box(modifier = Modifier.weight(1f).padding(8.dp)) {
            selectedTerrainIndex?.let { index ->
                _root_ide_package_.project.ui.TerrainEditor(
...
fun TerrainEditor(
    ...
    Column(modifier = Modifier.fillMaxWidth().verticalScroll(rememberScrollState())) {
        ...
        Row(
            ...
            if (mirrorCheck) {
                _root_ide_package_.project.ui.common.EnumDropdown(
        ...
        if (mirrorCheck.not()) {
            ...
            buildingCategories.forEach { (name, config) ->
                ...
                _root_ide_package_.project.ui.TerrainBuildingsConfigEditor(
                    config = config,
...
fun ZoneRandomizationConfigEditor(
    ...
    var showZoneSelectionDialog by remember { mutableStateOf<project.ui.ZoneSelectionContext?>(null) }
    ...
    LazyColumn(
        ...
        items(config?.ZonesToSwap ?: emptyList()) { zoneArray ->
            _root_ide_package_.project.ui.ZoneArrayEditor(
                zoneArray = zoneArray.toList(),
                onEditClicked = {
                    showZoneSelectionDialog = _root_ide_package_.project.ui.ZoneSelectionContext.ZonesToSwap(zoneArray)
                },
                onRemoveClicked = {
                    if (config != null)
                        onConfigChanged(
        ...
        item {
            Button(
                onClick = {
                    ...
                    showZoneSelectionDialog = _root_ide_package_.project.ui.ZoneSelectionContext.ZonesToSwap(newArray)
                },
                modifier = Modifier.fillMaxWidth()
            ) {
        ...
        items(config.ZonesToRandomize) { zoneId ->
            _root_ide_package_.project.ui.ZoneIdItem(
                zoneId = zoneId,
                onRemoveClicked = {
                    if (config != null)
                        onConfigChanged(
        ...
        item {
            Button(
                onClick = {
                    showZoneSelectionDialog = _root_ide_package_.project.ui.ZoneSelectionContext.ZonesToRandomize
                },
                modifier = Modifier.fillMaxWidth()
            ) {
    ...
    showZoneSelectionDialog?.let { context ->
        _root_ide_package_.project.ui.ZoneSelectionDialog(
            availableZoneIds = availableZoneIds,
            onDismiss = { showZoneSelectionDialog = null },
            onConfirm = { selectedIds ->
                when (context) {
                    is project.ui.ZoneSelectionContext.ZonesToSwap -> {
                        if (config != null) {
                    ...
                    _root_ide_package_.project.ui.ZoneSelectionContext.ZonesToRandomize -> {
                        if (config != null) {
            ...
            initialSelection = when (context) {
                is project.ui.ZoneSelectionContext.ZonesToSwap -> context.array.toList()
                _root_ide_package_.project.ui.ZoneSelectionContext.ZonesToRandomize -> emptyList()
            }
        )
    }
}
...
private sealed interface ZoneSelectionContext {
    data class ZonesToSwap(val array: IntArray) : project.ui.ZoneSelectionContext
    object ZonesToRandomize : project.ui.ZoneSelectionContext
}
